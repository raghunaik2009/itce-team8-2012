...
bool DistribNorm::computeMeanStd(double &mean,double &std,char mode, double percentH,double percentL){
	//HIEPNH - begin
	cout<<"DistribNorm::computeMeanStd" <<endl;
	//HIEPNH - end
...
double NormSeg::getMean(char computeMode,double discardH,double discardL){
	//HIEPNH - begin
	cout<<"NormSeg::getMean called" <<endl;
	//HIEPNH - end
...
void Norm::setNormAndFreeDistrib(){
	//HIEPNH - begin
	cout<<"Norm::setNormAndFreeDistrib() called" <<endl;
	//HIEPNH - end
...
int ComputeNorm(Config& config)
{
	...
	if(normType == "tnorm"){	
		...
		while ((linep=testList.getLine()) != NULL){
			...
			//HIEPNH - begin
			cout<< (linep->getElement(fieldLLR)).toDouble() <<" "<< tnorm.getMean(ind) <<" " << tnorm.getStd(ind) <<endl;
